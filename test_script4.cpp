...
void Shell::run() {
    ...
    std::cout << "Formatting and creating some test files (f1,f2) and
    directories (d1,d2)..." << std::endl; ret_val = filesystem.format(); if
    (ret_val) std::cout << "Error: format failed, error code " << ret_val <<
    std::endl; arg1 = "f1"; fw = open("input1.txt", O_RDONLY); dup2(fw, 0);
    ret_val = filesystem.create(arg1);
    if (ret_val)
        std::cout << "Error: create " << arg1 << " failed, error code " <<
    ret_val << std::endl; close(fw); arg1 = "f2"; fw = open("input2.txt",
    O_RDONLY); dup2(fw, 0); ret_val = filesystem.create(arg1); if (ret_val)
        std::cout << "Error: create " << arg1 << " failed, error code " <<
    ret_val << std::endl; close(fw); arg1 = "d1"; ret_val =
    filesystem.mkdir(arg1); if (ret_val) std::cout << "Error: mkdir(d1) " <<
    arg1 << " failed, error code " << ret_val << std::endl; arg1 = "d2"; ret_val
    = filesystem.mkdir(arg1); if (ret_val) std::cout << "Error: mkdir(d1) " <<
    arg1 << " failed, error code " << ret_val << std::endl; std::cout <<
    "Expected output:" << std::endl; std::cout << "/" << std::endl; std::cout <<
    "name\t type\t size" << std::endl; std::cout << "d1\t dir\t -" << std::endl;
    ...
    */
    ...
    std::cout << "Testing mkdir and cd with absolut and relative paths..."
              << std::endl;
    ...
    std::cout << "Testing cp/mv with absolut and relative paths..."
              << std::endl;
